...
10. Run focused tests, run the bundled audit against every maintained copy, and require the current CLI's mode-appropriate OpenAKB validation to succeed. Treat audit errors as blocking; review warnings individually. For an existing KB, use the supported explicit-target form so record-dependent checks cannot resolve an unintended target.
11. Build a separate publishing copy from every local blob referenced by the descriptor. The common set is `openakb.json`, `AKB.md`, and `sections/`; also include referenced file sources, captures, skills, and other local paths. Use the bundled audit's projection comparison to require byte-identical working and publishing copies, then recheck original-content integrity and validate both copies.
